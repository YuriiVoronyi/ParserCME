...
public class ShowBoardEOA extends BasePage {

    public ShowBoardEOA(WebDriver driver) {
        super(driver);
    }
    ...
    public ShowBoardEOA selectAllRows() {
    ...
    public ShowBoardEOA select10Rows() {
        WebElement dropdownMenu = driver.findElement(By.id("MainContent_ucViewControl_EssentialPricingSheet_ddlStrikeCount"));
        Select dropdown = new Select(dropdownMenu);
    ...
    public PricingSheets loadingBoardEOA() {
        select10Rows();
        pause(5000);
        selectAllRows();
        pause(5000);
        try {
            ...
            if (tblPricingSheet != null) {
                ...
                String deleteQuery = "DELETE FROM eoa_board";
                connection.prepareStatement(deleteQuery).executeUpdate();

                // Сброс автоинкрементного поля до начального значения
                String resetQuery = "ALTER TABLE eoa_board AUTO_INCREMENT = 1";
                connection.prepareStatement(resetQuery).executeUpdate();

                String query = "INSERT INTO eoa_board (id, volatility, delta_call, call_settle, `call`, strike, put, put_settle, delta_put, straddle, gamma, vega, theta) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            connection.close();
            System.out.println("Options board EOA has loaded");
        } catch (Exception e) {
