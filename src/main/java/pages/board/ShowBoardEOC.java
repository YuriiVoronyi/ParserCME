...
public class ShowBoardEOC extends BasePage {

    public ShowBoardEOC(WebDriver driver) {
        super(driver);
    }
    ...
    public ShowBoardEOC selectAllRows() {
    ...
    public ShowBoardEOC select10Rows() {
        WebElement dropdownMenu = driver.findElement(By.id("MainContent_ucViewControl_EssentialPricingSheet_ddlStrikeCount"));
        Select dropdown = new Select(dropdownMenu);
    ...
    public PricingSheets loadingBoardEOC() {
        select10Rows();
        pause(5000);
        selectAllRows();
        pause(5000);
        try {
            ...
            if (tblPricingSheet != null) {
                ...
                String deleteQuery = "DELETE FROM eoc_board";
                connection.prepareStatement(deleteQuery).executeUpdate();

                // Сброс автоинкрементного поля до начального значения
                String resetQuery = "ALTER TABLE eoc_board AUTO_INCREMENT = 1";
                connection.prepareStatement(resetQuery).executeUpdate();

                String query = "INSERT INTO eoc_board (id, volatility, delta_call, call_settle, `call`, strike, put, put_settle, delta_put, straddle, gamma, vega, theta) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            connection.close();
            System.out.println("Options board EOC has loaded");
        } catch (Exception e) {
