...
public class ShowBoardEOD extends BasePage {

    public ShowBoardEOD(WebDriver driver) {
        super(driver);
    }
    ...
    public ShowBoardEOD selectAllRows() {
    ...
    public ShowBoardEOD select10Rows() {
        WebElement dropdownMenu = driver.findElement(By.id("MainContent_ucViewControl_EssentialPricingSheet_ddlStrikeCount"));
        Select dropdown = new Select(dropdownMenu);
    ...
    public PricingSheets loadingBoardEOD() {
        select10Rows();
        pause(5000);
        selectAllRows();
        pause(5000);
        try {
            ...
            if (tblPricingSheet != null) {
                ...
                String deleteQuery = "DELETE FROM eod_board";
                connection.prepareStatement(deleteQuery).executeUpdate();

                // Сброс автоинкрементного поля до начального значения
                String resetQuery = "ALTER TABLE eod_board AUTO_INCREMENT = 1";
                connection.prepareStatement(resetQuery).executeUpdate();

                String query = "INSERT INTO eod_board (id, volatility, delta_call, call_settle, `call`, strike, put, put_settle, delta_put, straddle, gamma, vega, theta) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            connection.close();
            System.out.println("Options board EOD has loaded");
        } catch (Exception e) {
